"""
Logging Framework for omayabias
...
def _get_omayabias_logname():
    """
    Returns the path to the omayabias log file
    """
    if os.environ.has_key('HOME'):
        home = os.environ['HOME']
        if os.path.exists(os.path.join(home, '.omayabias')):
            fname = os.path.join(home, '.omayabias', 'omayabias.log')
        else:
            fname = os.path.join(home, 'omayabias.log')
        return fname
    else:
        return 'omayabias.log'
...
class NullHandler(pylogging.Handler):
    ...
    omayabias library can set up logging, and setup appropriate
    handlers, and then the omayabias library will appropriately
    handle correct logging.
    A omayabias module should do the following:
...
def add_file_handler(log, fname):
    ...
    #level = omayabiasParams['omayabias'].get('loglevel', 10)
...
logger = pylogging.getLogger('omayabias')
logger.logging = pylogging
logger.addHandler(NullHandler())
logger.will_debug = will_debug
add_file_handler(logger, _get_omayabias_logname())
